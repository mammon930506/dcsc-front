...
<template>
	<view class="wrap">
		<view class="fixedHeader">
			<u-icon style="font-size:40rpx;" name="arrow-left" @tap="gotoOrderList"></u-icon>
			支付取消<view></view>
		</view>
		...
		<view style="padding:0 34rpx 50rpx;position:relative">
			<view class="card" style="border-bottom-right-radius:0rpx;border-bottom-left-radius:0rpx;margin-bottom:0rpx">
				...
				<view class="authRow">
					...
					<view class="authContent nowrapellipsis">￥{{ cent2yuan(order.payAmount) }}</view>
				</view>
				<view class="authRow">
			...
			<view class="card"
				...
				<view>实付款：</view>
				<view style="color:#F87171">￥{{ cent2yuan(order.payAmount) }}</view>
			</view>
			<view class="card">
				<view class="authRow" style="margin-top:0">
					<view class="authTitle" style="width:100%">订单编号：{{ order.orderCode }}</view>
					<view class="authCopy" @tap="handleCopy(order.orderCode)">
						<view></view>
					</view>
				</view>
				<view class="authRow">
					<view class="authTitle" style="width:100%">下单时间：{{ order.payTime&&$u.timeFormat(order.createdDate, 'yyyy-mm-dd hh:MM:ss') }}</view>
				</view>
			<!-- 	<view class="authRow">
					<view class="authTitle" style="width:100%">支付方式：{{ order.orderCode }}</view>
				</view> -->
				<view class="authRow">
					<view class="authTitle" style="width:100%">支付时间：{{ order.payTime&&$u.timeFormat(order.payTime, 'yyyy-mm-dd hh:MM:ss') }}</view>
				</view>
			</view>
			...
			<view style="background-color:#2A2A34;height:200rpx;border-top:2rpx solid #59595D;padding:30rpx;position:fixed;width:100%;bottom:0;left:0">
				<view style="display:flex;justify-content: flex-end">
					<view class="button" @click="viewNft">查看尚品</view>
				</view>
			</view>
		</view>
	</view>
