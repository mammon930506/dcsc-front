...
var m_encKey = jlConfig.m_encKey;

//生成随机长度数
//再将其转为ascii码
function generalStringToAscii(length) {
  var strNumber = "";
  for (var i = 0; i < length; i++)
    strNumber = strNumber + (Math.floor(Math.random() * 10000) % 10).toString();
...
//十六进制字符串转byte
function hex2bytes(src) {
  if (src == null || src.length == 0 || src.length % 2 != 0) {
    return null;
  }
  var byteArrayResult = new Array();
  ...
  while (i < src.length - 1) {
    ...
    byteArrayResult[i / 2] = hex2byte(parseInt(strTemp, 16));
    i += 2;
  }
...
//encryptByDES("77c3052b141a481dd2f377c51571812c",'{dataInfo: "{"endDate":"2018-07-05","startDate":"2018-06-05"}"}');
//3des加密
...
function createContent(config) {
  ...
  // 生成随机数
  var randData = generalStringToAscii(32);  //"93DE52B773CA6C612075AC72DAE7B706";
...
export { createContent, s2as , resolveContent}